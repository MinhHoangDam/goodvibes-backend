...
// Health check endpoint - accepts GET, POST, and HEAD for compatibility with monitoring services
app.MapGet("/health", () =>
{
...
app.MapGet("/api/good-vibes/cached", async (GoodVibesCacheService cacheService, UserCacheService userCache, int? monthsBack, int? daysBack, string? avatarSize, bool? skipAvatars) =>
{
    try
    {
        ...
        // Enable avatar enrichment by default for progressive loading
        // Frontend loads 30-day chunks, so enrichment should complete quickly
        // Avatars can be skipped by passing skipAvatars=true if needed
...
public class UserCacheService
{
    ...
    public async Task<string?> GetUserAvatarAsync(string userId, string preferredSize = "32x32")
    {
        ...
        try
        {
            ...
            // - Failed fetches: cache empty string for 5 minutes to prevent hammering the API
            if (avatar != null)
            {
            ...
            {
                // Cache failure as empty string for 5 minutes to prevent repeated failed attempts
                _cache.Set(cacheKey, "", TimeSpan.FromMinutes(5));
            }
